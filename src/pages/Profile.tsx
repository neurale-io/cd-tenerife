...
export default function Profile() {
  ...
  if (view === 'join') {
    return <JoinScreen selected={plan} onSelect={setPlan} onContinue={() => setView('checkout')} onBack={() => setView('main')} />
  }
  if (view === 'checkout') {
    return (
      <CheckoutScreen
        plan={membershipPlans.find(p => p.id === plan)!}
        onSuccess={() => { setMember(true); setView('main') }}
        onBack={() => setView('join')}
      />
    )
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#060f22' }}>
      ...
      >
        <span style={{ fontSize: 16, fontWeight: 700, letterSpacing: '0.04em' }}>PERFIL</span>
      </div>

      <div className="scroll-area flex-1" style={{ padding: '20px 16px 28px' }}>
        ...
        {!isMember ? (
          <button
            ...
            style={{
              ...
              padding: '18px', borderRadius: 14, marginBottom: 20,
              background: '#0c1b3a', border: '1px solid rgba(212,167,38,0.3)',
            }}
          >
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 }}>
              <div>
                <p style={{ margin: '0 0 3px', fontSize: 17, fontWeight: 700 }}>Hazte Afiliado</p>
                <p style={{ margin: 0, fontSize: 13, color: 'rgba(255,255,255,0.5)' }}>Accede a la experiencia completa</p>
              </div>
              <span style={{ fontSize: 22, fontWeight: 900, color: '#d4a726' }}>5€</span>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 14 }}>
              {['Contenido exclusivo', 'Descuentos tienda', 'Acceso preferente', 'Sorteos y premios'].map(f => (
                <div key={f} style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                  <div style={{ width: 16, height: 16, borderRadius: 5, background: 'rgba(212,167,38,0.15)', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <svg width="8" height="6" viewBox="0 0 8 6" fill="none">
                      <path d="M1 3l2 2 4-4" stroke="#d4a726" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/>
                    </svg>
                  </div>
            ...
            <div style={{ background: '#d4a726', borderRadius: 8, padding: '10px 16px', textAlign: 'center' }}>
              <span style={{ fontSize: 14, fontWeight: 700, color: '#060f22' }}>HAZTE AFILIADO · 5€ / mes</span>
            </div>
          </button>
        ) : (
          <div style={{
            padding: '14px 16px', borderRadius: 12, marginBottom: 20,
            background: 'rgba(212,167,38,0.1)', border: '1px solid rgba(212,167,38,0.25)',
            display: 'flex', alignItems: 'center', gap: 12,
          }}>
            <div style={{ width: 40, height: 40, borderRadius: 10, background: 'rgba(212,167,38,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
                <circle cx="9" cy="9" r="8" stroke="#d4a726" strokeWidth="1.5"/>
                <path d="M5.5 9l2.5 2.5 4-4.5" stroke="#d4a726" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
            </div>
            <div style={{ flex: 1 }}>
              <p style={{ margin: '0 0 2px', fontSize: 14, fontWeight: 700 }}>Blue Member · Activo</p>
              <p style={{ margin: 0, fontSize: 12, color: 'rgba(255,255,255,0.45)' }}>Renueva el 10 Jul 2026 · 5€/mes</p>
            </div>
          </div>
        )}
        ...
        <div style={{ borderRadius: 12, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.07)', marginBottom: 12 }}>
          {[
            { icon: CreditCard, label: 'Afiliación y Facturación', sub: isMember ? 'Blue Member · Activo' : 'No afiliado', action: () => !isMember && setView('join') },
            { icon: Bell,       label: 'Notificaciones',           sub: 'Notificaciones push activadas' },
            { icon: Settings,   label: 'Ajustes de cuenta',        sub: 'Idioma, privacidad, seguridad' },
            { icon: HelpCircle, label: 'Ayuda y soporte',          sub: 'Preguntas frecuentes y contacto' },
        ...
        <button style={{
          ...
          <span style={{ fontSize: 14, fontWeight: 600, color: '#e74c3c' }}>Cerrar sesión</span>
        </button>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#060f22' }}>
      ...
      <div className="scroll-area flex-1" style={{ padding: '24px 16px 20px' }}>
        <p style={{ margin: '0 0 24px', fontSize: 14, color: 'rgba(255,255,255,0.55)', lineHeight: 1.6 }}>
          Únete al CD Tenerife y accede a contenido exclusivo, descuentos en tienda y mucho más.
        </p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 24 }}>
          {membershipPlans.map(p => (
            <button
              ...
              style={{
                display: 'block', width: '100%', textAlign: 'left', cursor: 'pointer',
                padding: '18px', borderRadius: 14, background: '#0c1b3a',
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 14 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                  <div>
                    ...
                    <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)' }}>/{p.period === 'month' ? 'mes' : 'año'}</span>
                  </div>
                  <div style={{
        ...
        <button className="btn btn-primary" style={{ width: '100%', padding: '15px 0', borderRadius: 12, fontSize: 15, fontWeight: 700 }} onClick={onContinue}>
          Continuar al pago
        </button>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#060f22' }}>
      ...
      <div className="scroll-area flex-1" style={{ padding: '20px 16px 0' }}>
        <div style={{ background: '#0c1b3a', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: '14px 16px', marginBottom: 20 }}>
          <p style={{ margin: '0 0 12px', fontSize: 11, fontWeight: 600, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Resumen del pedido</p>
        ...
        <div style={{ background: '#0c1b3a', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: '14px 16px', marginBottom: 20 }}>
          <p style={{ margin: '0 0 16px', fontSize: 11, fontWeight: 600, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Datos de la tarjeta</p>
      ...
      <div style={{
        ...
        >
          {loading ? (
            ...
            <><CreditCard size={16} /> Pagar €{plan.price} · Activar afiliación</>
          )}
        </button>
